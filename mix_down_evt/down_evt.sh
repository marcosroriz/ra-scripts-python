...
while true; do
  (
    ...
    # Today
    dia_hoje=$(date +%Y-%m-%d)
    python -u /app/down_evt.py --data_baixar="$dia_hoje" >> "${LOG_DIR}/${dia_hoje}-evt.txt" 2>&1 || echo "[error] failed for $dia_hoje"

    # -1
    dia_anterior=$(date -d "-1 day" +%Y-%m-%d)
    python -u /app/down_evt.py --data_baixar="$dia_anterior" >> "${LOG_DIR}/${dia_anterior}-evt-dia-anterior.txt" 2>&1 || echo "[error] failed for $dia_anterior"
