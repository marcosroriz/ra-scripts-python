...
def preprocessa_linhas(df_linhas_raw):
    ...
    # Adiciona campo VIGENCIA_VALIDA se a VIGENCIA_FINAL > dia_atual
    # df_linhas_raw["VIGENCIA_FINAL_DT"] = pd.to_datetime(df_linhas_raw["VIGENCIA_FINAL"], format="%d/%m/%Y")
    # df_linhas_raw["VIGENCIA_VALIDA"] = df_linhas_raw["VIGENCIA_FINAL_DT"].dt.date >= dt.date.today()
...
@click.command()
@click.option("--kml", type=str, help="Arquivo KML", required=True)
@click.option("--nome", type=str, help="Nome da Linha")
def main(kml, nome):
    ...
    if not os.path.isfile(kml):
        print(f"Arquivo KML não encontrado: {kml}")
        sys.exit(1)

    # Lê o conteúdo do KML (pode ser usado posteriormente)
    with open(kml, "r", encoding="utf-8") as f:
        kml_linha_unica_str = f.read()
        ...
        # Nome da Linha
        if not nome:
            kml_nome_linha = kml_linha_unica_geojson["features"][0]["properties"]["name"]
            km_nome_linha_normalizado = unidecode(kml_nome_linha).upper().replace(" ", "")
            nome = km_nome_linha_normalizado
        ...
        try:
            with pg_engine.begin() as conn:
                data_dict = {
                    "diahorario": today,
                    "dia_vigencia_inicial": "01/01/2025",
                    "dia_vigencia_final": "01/09/2099",
                    "numero": nome,
                    "numero_sublinha": nome,
                    "desc_linha": nome,
                    "sentido": "IDA",
                ...
                print(f"Linha {nome} inserida com sucesso")
        except Exception as e:
            print(f"Erro ao processar sub_linha {nome}: {e}")
