...
def buscar_dados_banco(periodo):
    df_fuel_consumption = db_fetcher.buscar_dados_combustivel(periodo)
    str_csv_fuel_consumption = convert_df_para_csv(df_fuel_consumption)

    df_pecas_trocadas = db_fetcher.buscar_pecas_trocadas_top_veiculos(periodo)
    str_csv_pecas_trocadas = convert_df_para_csv(df_pecas_trocadas)

    df_os_veiculos = db_fetcher.buscar_os_top_veiculos(periodo)
    str_csv_os_veiculos = convert_df_para_csv(df_os_veiculos)
    return str_csv_fuel_consumption, str_csv_pecas_trocadas, str_csv_os_veiculos
...
if __name__ == "__main__":
    ...
    str_csv_fuel_consumption, str_csv_pecas_trocadas, str_csv_os_veiculos = buscar_dados_banco(periodo=7)

    # Gera os Inputs
    system_input = get_system_instructions("Relatório de Combustível", 7, None, None)
    user_input = get_user_prompt(
        None,
        str_csv_fuel_consumption,
        str_csv_pecas_trocadas,
        str_csv_os_veiculos,
    )
    ...
    try:
        ...
        print(e)
        print(f"Erro ao gerar o relatório para a regra {relatorio_md}: {e}")
